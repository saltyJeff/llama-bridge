...
public:
    ...
    Value get(const Value &key)
    {
        ...
        {
            if (!key.is_hashable())
                throw std::runtime_error("Unhashable type: " + dump());
    ...
    const Value &at(const Value &index) const { return const_cast<Value *>(this)->at(index); }
    ...
    const Value &at(size_t index) const { return const_cast<Value *>(this)->at(index); }
    ...
    Value operator%(const Value &rhs) const { return get<int64_t>() % rhs.get<int64_t>(); }
};
...
struct ArgumentsValue
{
    ...
    {
        if (args.size() < pos_count.first || args.size() > pos_count.second || kwargs.size() < kw_count.first ||
            kwargs.size() > kw_count.second)
        {
            std::ostringstream out;
            out << method_name << " must have between " << pos_count.first << " and " << pos_count.second
                << " positional arguments and between " << kw_count.first << " and " << kw_count.second
                << " keyword arguments";
...
template <> inline json Value::get<json>() const
{
    if (is_primitive())
        return primitive_;
    if (is_null())
...
namespace std
{
template <> struct hash<minja::Value>
{
...
public:
    ...
    virtual void set(const Value &key, const Value &value) { values_.set(key, value); }
};
...
class VariableExpr : public Expression
{
    std::string name;
public:
    VariableExpr(const Location &loc, const std::string &n) : Expression(loc), name(n) {}
...
static void destructuring_assign(const std::vector<std::string> &var_names, const std::shared_ptr<Context> &context,
                                 Value &item)
{
    if (var_names.size() == 1)
    {
...
struct MacroTemplateToken : public TemplateToken
{
    ...
    MacroTemplateToken(const Location &loc, SpaceHandling pre, SpaceHandling post, std::shared_ptr<VariableExpr> &&n,
...
struct ForTemplateToken : public TemplateToken
{
    ...
    std::shared_ptr<Expression> condition;
    bool recursive;
    ForTemplateToken(const Location &loc, SpaceHandling pre, SpaceHandling post, const std::vector<std::string> &vns,
                     std::shared_ptr<Expression> &&iter, std::shared_ptr<Expression> &&c, bool r)
        : TemplateToken(Type::For, loc, pre, post), var_names(vns), iterable(std::move(iter)), condition(std::move(c)),
          recursive(r)
    {
    }
};
...
struct SetTemplateToken : public TemplateToken
{
    ...
    SetTemplateToken(const Location &loc, SpaceHandling pre, SpaceHandling post, const std::string &ns,
...
public:
    ...
    LoopControlException(LoopControlType control_type)
        : std::runtime_error((control_type == LoopControlType::Continue ? "continue" : "break") +
...
struct LoopControlTemplateToken : public TemplateToken
{
    ...
    LoopControlTemplateToken(const Location &loc, SpaceHandling pre, SpaceHandling post, LoopControlType control_type)
        : TemplateToken(Type::Break, loc, pre, post), control_type(control_type)
    {
    }
};
...
public:
    ...
    void do_render(std::ostringstream &out, const std::shared_ptr<Context> &) const override { out << text; }
};
...
public:
    ForNode(const Location &loc, std::vector<std::string> &&var_names, std::shared_ptr<Expression> &&iterable,
            std::shared_ptr<Expression> &&condition, std::shared_ptr<TemplateNode> &&body, bool recursive,
            std::shared_ptr<TemplateNode> &&else_body)
        : TemplateNode(loc), var_names(var_names), iterable(std::move(iterable)), condition(std::move(condition)),
          body(std::move(body)), recursive(recursive), else_body(std::move(else_body))
    {
    }
...
public:
    MacroNode(const Location &loc, std::shared_ptr<VariableExpr> &&n, Expression::Parameters &&p,
    ...
    void do_render(std::ostringstream &, const std::shared_ptr<Context> &context) const override
    {
        ...
        auto callable = Value::callable([weak_context = std::weak_ptr<Context>(context), name = name, params = params,
                                         body = body, named_param_positions = named_param_positions](
                                            const std::shared_ptr<Context> &call_context, ArgumentsValue &args) {
            auto context_locked = weak_context.lock();
            if (!context_locked)
                throw std::runtime_error("Macro context no longer valid");
            ...
            for (size_t i = 0, n = args.args.size(); i < n; i++)
            {
                ...
                if (i >= params.size())
                    throw std::runtime_error("Too many positional arguments for macro " + name->get_name());
                param_set[i] = true;
            ...
            for (auto &[arg_name, value] : args.kwargs)
            {
                ...
                if (it == named_param_positions.end())
                    throw std::runtime_error("Unknown parameter name for macro " + name->get_name() + ": " + arg_name);
...
public:
    IfExpr(const Location &loc, std::shared_ptr<Expression> &&c, std::shared_ptr<Expression> &&t,
...
public:
    ...
    Value do_evaluate(const std::shared_ptr<Context> &context) const override
    {
        ...
        if (auto slice = dynamic_cast<SliceExpr *>(index.get()))
        {
            ...
            int64_t start =
                slice->start ? wrap(slice->start->evaluate(context).get<int64_t>()) : (step < 0 ? len - 1 : 0);
...
public:
    ...
    std::shared_ptr<Expression> expr;
    Op op;
    UnaryOpExpr(const Location &loc, std::shared_ptr<Expression> &&e, Op o) : Expression(loc), expr(std::move(e)), op(o)
    {
    }
    Value do_evaluate(const std::shared_ptr<Context> &context) const override
    {
...
{
    return (
        ((container.is_array() || container.is_object()) && container.contains(value)) ||
        (value.is_string() && container.is_string() && container.to_str().find(value.to_str()) != std::string::npos));
}
        ...
        for (const auto &arg : this->args)
        {
            if (auto un_expr = std::dynamic_pointer_cast<UnaryOpExpr>(arg))
            {
                ...
                else if (un_expr->op == UnaryOpExpr::Op::ExpansionDict)
                {
                    ...
                    dict.for_each(
                        [&](const Value &key) { vargs.kwargs.push_back({key.get<std::string>(), dict.at(key)}); });
                    continue;
                }
            }
...
public:
    MethodCallExpr(const Location &loc, std::shared_ptr<Expression> &&obj, std::shared_ptr<VariableExpr> &&m,
    ...
    Value do_evaluate(const std::shared_ptr<Context> &context) const override
    {
        ...
        else if (obj.is_string())
        {
            ...
            else if (method->get_name() == "replace")
            {
                ...
                auto count = vargs.args.size() == 3 ? vargs.args[2].get<int64_t>() : str.length();
                size_t start_pos = 0;
                while ((start_pos = str.find(before, start_pos)) != std::string::npos && count-- > 0)
                {
...
public:
    ...
    {
        if (!expr)
            throw std::runtime_error("CallNode.expr is null");
        if (!body)
            throw std::runtime_error("CallNode.body is null");

        // Use init-capture to avoid dangling 'this' pointer and circular references
        auto caller = Value::callable([weak_context = std::weak_ptr<Context>(context),
                                       body = body](const std::shared_ptr<Context> &, ArgumentsValue &) -> Value {
            auto context_locked = weak_context.lock();
            if (!context_locked)
                throw std::runtime_error("Caller context no longer valid");
...
public:
    FilterExpr(const Location &loc, std::vector<std::shared_ptr<Expression>> &&p) : Expression(loc), parts(std::move(p))
    {
    }
    Value do_evaluate(const std::shared_ptr<Context> &context) const override
    {
    ...
    void prepend(std::shared_ptr<Expression> &&e) { parts.insert(parts.begin(), std::move(e)); }
};
...
private:
    ...
    public:
        ...
        size_t get_pos(const CharIterator &begin) const { return std::distance(begin, it); }
    };
    ...
    Location get_location() const { return {template_str, (size_t)std::distance(start, it)}; }
    ...
    std::shared_ptr<Expression> parseLogicalCompare()
    {
        ...
        while (!(op_str = consumeToken(compare_tok)).empty())
        {
            ...
            if (op_str == "is")
            {
                ...
                return std::make_shared<BinaryOpExpr>(left->location, std::move(left), std::move(identifier),
    ...
    std::shared_ptr<Expression> parseStringConcat()
    {
        ...
        if (!consumeToken(concat_tok).empty())
        {
            ...
            left = std::make_shared<BinaryOpExpr>(get_location(), std::move(left), std::move(right),
    ...
    {
        return std::make_shared<CallExpr>(get_location(), std::make_shared<VariableExpr>(get_location(), name),
                                          std::move(args));
    }
        ...
        try
        {
            while (it != end)
            {
                ...
                else if (!(group = consumeTokenGroups(expr_open_regex, SpaceHandling::Keep)).empty())
                {
                    ...
                    tokens.push_back(
                        std::make_unique<ExpressionTemplateToken>(location, pre_space, post_space, std::move(expr)));
                }
                else if (!(group = consumeTokenGroups(block_open_regex, SpaceHandling::Keep)).empty())
                {
                    ...
                    if (keyword == "if")
                    {
                        ...
                        tokens.push_back(
                            std::make_unique<IfTemplateToken>(location, pre_space, post_space, std::move(condition)));
                    }
                    else if (keyword == "elif")
                    {
                        ...
                        auto post_space = parseBlockClose();
                        tokens.push_back(
                            std::make_unique<ElifTemplateToken>(location, pre_space, post_space, std::move(condition)));
                    }
                    ...
                    else if (keyword == "for")
                    {
                        ...
                        auto post_space = parseBlockClose();
                        tokens.push_back(std::make_unique<ForTemplateToken>(location, pre_space, post_space,
                                                                            std::move(varnames), std::move(iterable),
                                                                            std::move(condition), recursive));
                    }
                    ...
                    else if (keyword == "set")
                    {
                        ...
                        auto post_space = parseBlockClose();
                        tokens.push_back(std::make_unique<SetTemplateToken>(location, pre_space, post_space, ns,
                                                                            var_names, std::move(value)));
                    }
                    ...
                    else if (keyword == "macro")
                    {
                        ...
                        auto post_space = parseBlockClose();
                        tokens.push_back(std::make_unique<MacroTemplateToken>(location, pre_space, post_space,
                                                                              std::move(macroname), std::move(params)));
                    }
                    ...
                    else if (keyword == "call")
                    {
                        auto expr = parseExpression();
                        if (!expr)
                            throw std::runtime_error("Expected expression in call block");

                        auto post_space = parseBlockClose();
                        tokens.push_back(
                            std::make_unique<CallTemplateToken>(location, pre_space, post_space, std::move(expr)));
                    }
                    ...
                    else if (keyword == "filter")
                    {
                        ...
                        auto post_space = parseBlockClose();
                        tokens.push_back(
                            std::make_unique<FilterTemplateToken>(location, pre_space, post_space, std::move(filter)));
                    }
                    ...
                    else if (keyword == "break" || keyword == "continue")
                    {
                        ...
                        tokens.push_back(std::make_unique<LoopControlTemplateToken>(
                            ...
                            keyword == "break" ? LoopControlType::Break : LoopControlType::Continue));
                    }
                    else
                    {
                ...
                else if (std::regex_search(it, end, match, non_text_open_regex))
                {
                    ...
                    it = text_end;
                    tokens.push_back(
                        std::make_unique<TextTemplateToken>(location, SpaceHandling::Keep, SpaceHandling::Keep, text));
                }
                else
                {
    ...
    std::shared_ptr<TemplateNode> parseTemplate(const TemplateTokenIterator &begin, TemplateTokenIterator &it,
        ...
        while (it != end)
        {
            ...
            else if (auto for_token = dynamic_cast<ForTemplateToken *>(token.get()))
            {
                ...
                children.emplace_back(std::make_shared<ForNode>(
                    token->location, std::move(for_token->var_names), std::move(for_token->iterable),
                    std::move(for_token->condition), std::move(body), for_token->recursive, std::move(else_body)));
            }
            else if (dynamic_cast<GenerationTemplateToken *>(token.get()))
            {
                ...
                // Treat as a no-op, as our scope is templates for inference, not training (`{% generation %}` wraps
                // generated tokens for masking).
            ...
            else if (auto set_token = dynamic_cast<SetTemplateToken *>(token.get()))
            {
                ...
                else
                {
                    ...
                    children.emplace_back(
                        std::make_shared<SetTemplateNode>(token->location, name, std::move(value_template)));
                }
            }
            else if (auto macro_token = dynamic_cast<MacroTemplateToken *>(token.get()))
            {
                ...
                children.emplace_back(std::make_shared<MacroNode>(token->location, std::move(macro_token->name),
            ...
            else if (auto call_token = dynamic_cast<CallTemplateToken *>(token.get()))
            {
                ...
                children.emplace_back(
                    std::make_shared<CallNode>(token->location, std::move(call_token->expr), std::move(body)));
            }
            else if (auto filter_token = dynamic_cast<FilterTemplateToken *>(token.get()))
            {
                ...
                children.emplace_back(
                    std::make_shared<FilterNode>(token->location, std::move(filter_token->filter), std::move(body)));
            }
...
inline std::shared_ptr<Context> Context::builtins()
{
    ...
    globals.set("tojson",
                simple_function(
                    "tojson", {"value", "indent", "ensure_ascii"}, [](const std::shared_ptr<Context> &, Value &args) {
    ...
    globals.set("capitalize",
                simple_function("capitalize", {"text"}, [](const std::shared_ptr<Context> &, Value &args) {
                    auto &text = args.at("text");
    ...
    globals.set(
        "default", Value::callable([=](const std::shared_ptr<Context> &, ArgumentsValue &args) {
    ...
    globals.set("joiner", simple_function("joiner", {"sep"}, [](const std::shared_ptr<Context> &, Value &args) {
                    ...
                    return simple_function("", {},
                                           [sep, first](const std::shared_ptr<Context> &, const Value &) -> Value {
                                               if (*first)
                                               {
                                                   *first = false;
                                                   return "";
    ...
    globals.set("dictsort", simple_function("dictsort", {"value"}, [](const std::shared_ptr<Context> &, Value &args) {
                    if (args.size() != 1)
                        throw std::runtime_error("dictsort expects exactly 1 argument (TODO: fix implementation)");
    ...
    globals.set("join", simple_function("join", {"items", "d"}, [](const std::shared_ptr<Context> &, Value &args) {
                    ...
                    {
                        return simple_function(
                            "", {"items"}, [sep, do_join](const std::shared_ptr<Context> &, Value &args) {
                                auto &items = args.at("items");
                                if (!items.to_bool() || !items.is_array())
                                    throw std::runtime_error("join expects an array for items, got: " + items.dump());
                                return do_join(items, sep);
                            });
                    }
                }));
    ...
    auto equalto =
        simple_function("equalto", {"expected", "actual"}, [](const std::shared_ptr<Context> &, Value &args) -> Value {
            return args.at("actual") == args.at("expected");
        });
    globals.set("equalto", equalto);
    globals.set("==", equalto);
    globals.set("length",
                simple_function("length", {"items"}, [](const std::shared_ptr<Context> &, Value &args) -> Value {
                    auto &items = args.at("items");
    ...
    globals.set("string",
                simple_function("string", {"value"}, [](const std::shared_ptr<Context> &, Value &args) -> Value {
                    return args.at("value").to_str();
                }));
    ...
    globals.set("in",
                simple_function("in", {"item", "items"}, [](const std::shared_ptr<Context> &, Value &args) -> Value {
                    return in(args.at("item"), args.at("items"));
                }));
    globals.set("unique",
                simple_function("unique", {"items"}, [](const std::shared_ptr<Context> &, Value &args) -> Value {
                    auto &items = args.at("items");
                    if (!items.is_array())
                        throw std::runtime_error("object is not iterable");
    ...
    globals.set("map", Value::callable([=](const std::shared_ptr<Context> &context, ArgumentsValue &args) {
                    auto res = Value::array();
                    if (args.args.size() == 1 && ((args.has_named("attribute") && args.kwargs.size() == 1) ||
